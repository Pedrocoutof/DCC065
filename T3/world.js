...
export class World extends THREE.Group {
  ...
  generateMesh() {
    const grassMesh = this.generateMeshVoxel("grass");
    grassMesh.count = 0;
    const dirtMesh = this.generateMeshVoxel("dirt");
    dirtMesh.count = 0;
    const stoneMesh = this.generateMeshVoxel("stone");
    stoneMesh.count = 0;
    const sandMesh = this.generateMeshVoxel("sand");
    sandMesh.count = 0;
    const waterMesh = this.generateMeshVoxel("water"); // Adicionar água
    waterMesh.count = 0;
    ...
    const typesMeshes = {
      ...
      stone: stoneMesh,
      sand: sandMesh,
      water: waterMesh, // Adicionar água
    };
    ...
    this.add(stoneMesh);
    this.add(sandMesh);
    this.add(waterMesh); // Adicionar água
  }
    ...
    buildModeling(data, basePosition) {
      data.forEach((voxelData) => {
          ...
          if (voxelData.additionalData.name === "oak") {
              ...
              voxelMaterial = setDefaultMaterial(voxelData.materialProps.color || 0xffffff);
          }
  ...
  autumnThree(x, y, z) {
    ...
    const trunkMaterial = new THREE.MeshLambertMaterial({ map: this.wood1Texture }); // Usando wood1
...
defaultThree(x, y, z) {
  const trunkMaterial = new THREE.MeshLambertMaterial({ map: this.wood2Texture }); // Usando wood2
